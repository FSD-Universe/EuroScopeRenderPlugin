...
| **size**                      | 整数  | 12        | 参考字号（在插件设置 **TextSizeReferenceZoom** 指定的 zoom 下为参考像素，默认 12）；随缩放连续变化以保持相对大小 |
...
| **TextSizeReferenceZoom** | `12`               | 文字 **size** 的参考缩放等级（1–19），该 zoom 下配置的 size 即对应参考像素；用于计算文字随缩放的显示大小。 |
